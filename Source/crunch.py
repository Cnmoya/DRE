from . import ModelsIO as MIO
import numpy as np
from h5py import File

def E_fit( _cube:np.ndarray((10,13,128,21,128),'>f4'), data:np.ndarray((10,13,128,21,128),'>f4'), seg:np.ndarray((10,13,128,21,128),'>f4'),noise:np.ndarray((10,13,128,21,128),'>f4')) -> np.ndarray((10,13,21),">f4"):
    #seg_cube, data_cube,flux_models,flux_data,X,chi

    
    #seg_cube:np.ndarray((10,13,21),">f4")
    #data_cube:np.ndarray((10,13,21),">f4")
    #data_re : np.ndarray((10,13,128,21,128),'>f4')
    #seg_re : np.ndarray((10,13,128,21,128),'>f4')
    scaled_models : np.ndarray((10,13,128,21,128),'>f4')
    flux_models:np.ndarray((10,13,21),">f4")
    flux_data:np.ndarray((10,13,21),">f4")
    X:np.ndarray((10,13,21),">f4")
    resta : np.ndarray((10,13,128,21,128),'>f4')
    residuo : np.ndarray((10,13,128,21,128),'>f4')
    chi:np.ndarray((10,13,21),">f4")
    area : int

    #seg_cube = np.tile(seg,(10,13,21))
    #data_cube = np.tile(data,(10,13,21))
    #data_re=data_cube.reshape((10,13,128,21,128))
    #seg_re=seg_cube.reshape((10,13,128,21,128))
    flux_models = np.einsum("ijkla->ijl",_cube*seg)
    flux_data = np.einsum("ijkla->ijl",data*seg)
    X = flux_data/flux_models
    scaled_models=np.einsum("ijk,ijakb->ijakb",X,_cube)
    resta = data-scaled_models
    #cambio algo aca, como sumo el ruido
    residuo = (resta**2)/(np.sqrt((scaled_models**2+noise**2)))
    chi = np.einsum("ijakb->ijk",residuo*seg)
